from project.food_factory import FoodFactory
from project.drink_factory import DrinkFactory
from project.table_factory import TableFactory
...
class Bakery:
    ...
    def order_food(self, table_number: int, *food_names: []):
        ...
        skipped_orders = f"{self.name} does not have in the menu:\n"

        for food_name in food_names:
            food = self.find_food_by_name(food_name)
            if food is None:
                skipped_orders += food_name + '\n'
            else:
                table.order_food(food)
                ordered_food += str(food) + '\n'
        return ordered_food + skipped_orders.strip()
    ...
    def get_free_tables_info(self):
        ...
        for table in self.tables_repository:
            if not table.is_reserved:
                result += str(table.free_table_info()) + '\n'
        return result.strip()
